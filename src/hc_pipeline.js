...
const CACHE_TTL_MS = 2618000; // φ² × 1000 ms (~43.6 min)
const CACHE_MAX_ENTRIES = 233; // fib(13)
...
async function executeTask(taskName, context) {
  ...
  // FAIL-CLOSED: no handler = real failure, not simulated success
  // This ensures the pipeline never silently fakes work
  throw new Error(
    `[FAIL-CLOSED] No handler registered for task '${taskName}'. ` +
    `Register via registerTaskHandler('${taskName}', fn) before pipeline execution.`
  );
}
