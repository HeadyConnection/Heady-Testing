...
function wireEngines(app, deps = {}) {
    ...
    try {
        ...
        // BUG-30: Load HCFullPipeline for autonomous self-healing loop
        let hcfpInstance = null;
        try {
            const HCFullPipeline = require("../orchestration/hc-full-pipeline");
            hcfpInstance = new HCFullPipeline({
                vectorMemory: deps.vectorMemory || null,
            });
            logger.logNodeActivity("CONDUCTOR", "  ∞ HCFullPipeline: LOADED (21-stage autonomous pipeline)");
        } catch (pipelineErr) {
            logger.logNodeActivity("CONDUCTOR", `  ⚠ HCFullPipeline not loaded: ${pipelineErr.message}`);
        }
        ...
        engines.autoSuccessEngine.wire({
            ...
            eventBus: eventBus,
            pipeline: hcfpInstance,  // BUG-30: Wire HCFP for autonomous self-healing
        });
        ...
        logger.logNodeActivity("CONDUCTOR", `  ∞ Auto-Success Engine: LOADED (ALL tasks, dynamic parallel — no batching${hcfpInstance ? ', HCFP autonomous loop ACTIVE' : ''})`);
