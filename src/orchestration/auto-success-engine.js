...
class AutoSuccessEngine {
  ...
  async _runWithRetry(category, taskId, fn) {
    ...
    // Defensive: should never reach here, but satisfy static analysis
    return { task: taskId, status: 'fail', value: { category, unreachable: true }, durationMs: Date.now() };
  }
  ...
  async _runCycle() {
    ...
    // ═══ LAW-07 FULL AUTONOMY: Trigger HCFP if health is degraded ═══
    const health = this.getHealth();
    if (health.status !== 'HEALTHY') {
        try {
            const pipelineRunner = require('../hcfp/pipeline-runner');
            const failedTasks = results.filter(r => r.status !== 'pass');
            
            // BUG-19 guard: schema rejects empty tasks array — only dispatch if failures exist
            if (failedTasks.length > 0) {
                // Construct autonomous healing manifest based on failures
                const manifest = {
                    name: `Autonomous Healing Cycle ${this._metrics.cycleCount}`,
                    priority: 'high',
                    tasks: failedTasks.map((t, idx) => ({
                        id: `heal-${t.taskId}-${idx}`,
                        name: `Auto-Heal: ${t.taskId}`,
                        action: 'fix',
                        service_group: 'core',
                        inputs: {
                            taskId: t.taskId,
                ...
                // Fire and forget pipeline execution — don't block the next success cycle
                pipelineRunner.runFull(manifest).catch(err => {
                    // Keep engine running even if pipeline errors out
                });
                this._metrics.learningEvents++;
            }
        } catch (err) {
            // Failsafe — if pipeline runner is not available, log internally
        }
    }
  ...
  getMetrics() {
    ...
    return {
      ...
      totalTasks:      Object.values(TASK_REGISTRY).reduce((sum, tasks) => sum + Object.keys(tasks).length, 0),
  ...
  /**
   * getStatus() — returns engine status for /api/auto-success/status.
   */
  getStatus() {
    return {
      ...
      cycleIntervalMs: CYCLE_INTERVAL_MS,
      uptimeSec: parseFloat(((Date.now() - this._startTime) / 1000).toFixed(1))
    };
  }

  /**
   * getHistory(limit) — returns recent cycle history for /api/auto-success/history.
   */
  getHistory(limit = 50) {
  ...
  /**
   * getTaskCatalog(category) — returns task catalog for /api/auto-success/tasks.
   */
  getTaskCatalog(category = null) {
    const entries = category
      ? { [category]: TASK_REGISTRY[category] || {} }
      : TASK_REGISTRY;
    return Object.entries(entries).flatMap(([cat, tasks]) =>
      Object.keys(tasks).map(taskId => ({ category: cat, taskId }))
    );
  }
}
