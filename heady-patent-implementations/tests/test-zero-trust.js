...
test('shannonEntropy of varied string > 3', () => {
  const entropy = shannonEntropy('TESTKEY0000000000000');
  assert.ok(entropy > 3);
});
...
test('SecretScanner detects AWS access key', () => {
  ...
  const findings = scanner.scan('key=TESTKEY0000000000000 here');
...
test('SecretScanner getFindings accumulates', () => {
  const scanner = new SecretScanner();
  scanner.scan('TESTKEY0000000000000');
...
test('OutputSanitizer redacts secrets', () => {
  const san    = new OutputSanitizer();
  const result = san.sanitize('Using key TESTKEY0000000000000 to access AWS');
...
test('AuditTrail redacts secrets from log entries', () => {
  const trail = new AuditTrail();
  trail.append('store_key', { key: 'TESTKEY0000000000000ABCDEF1234567890XYZ' });
  const entries = trail.query({ action: 'store_key' });
  assert.ok(!JSON.stringify(entries).includes('TESTKEY0000000000000'));
});
...
test('SanitizationPipeline blocks AWS key', () => {
  const pipeline = new SanitizationPipeline({ strict: true });
  const result   = pipeline.run('My key is TESTKEY0000000000000 please use it');
