...
exports.PHI_TIMING = {
    ...
    CYCLE: Math.round(Math.pow(exports.PHI, 7) * 1000), // 29,034ms — alias for PHI_7 (heartbeat cycle)
};
// ─── CSL GATE THRESHOLDS ────────────────────────────────────────────────────
...
exports.AUTO_SUCCESS = {
    ...
    CATEGORIES: 9, // actual implemented categories (scaling toward fib(7)=13)
    CATEGORIES_TARGET: fib(7), // 13 — φ-target for future expansion
    TASKS_TOTAL: 135, // actual implemented tasks (scaling toward fib(12)=144)
    TASKS_TOTAL_TARGET: fib(12), // 144 — φ-target for future expansion
    TASKS_PER_CATEGORY: 15, // actual tasks per category
...
// ─── DERIVED CONSTANTS (consumed by tests and utilities) ────────────────────
exports.PHI_CB = exports.PHI_CUBED; // alias: PHI³
exports.PHI_TEMPERATURE = Math.pow(exports.PSI, 3); // ψ³ ≈ 0.236
exports.DEDUP_THRESHOLD = exports.VECTOR.DEDUP_THRESHOLD;
// ─── UTILITY FUNCTIONS ──────────────────────────────────────────────────────
const EPSILON = 1e-10;
function sigmoid(x) {
...
function cslGate(value, cosScore, tau = exports.CSL_THRESHOLDS.MEDIUM, temp = exports.PHI_TEMPERATURE) {
    const x = (cosScore - tau) / (temp + EPSILON);
    return value * sigmoid(x);
}
exports.cslGate = cslGate;
function cslBlend(weightHigh, weightLow, cosScore, tau = exports.CSL_THRESHOLDS.MEDIUM) {
    const x = (cosScore - tau) / (exports.PHI_TEMPERATURE + EPSILON);
    const t = sigmoid(x);
    return weightLow + (weightHigh - weightLow) * t;
}
exports.cslBlend = cslBlend;
...
exports.phiPriorityScore = phiPriorityScore;
function phiTokenBudgets(base = 8192) {
    return {
        working:   Math.round(base),
        session:   Math.round(base * Math.pow(exports.PHI, 2)),
        memory:    Math.round(base * Math.pow(exports.PHI, 4)),
        artifacts: Math.round(base * Math.pow(exports.PHI, 6)),
    };
}
exports.phiTokenBudgets = phiTokenBudgets;
function phiAdaptiveInterval(currentInterval, healthy, minMs = 1000, maxMs = 60000) {
    if (healthy) {
        return Math.min(maxMs, Math.round(currentInterval * exports.PHI));
    } else {
        return Math.max(minMs, Math.round(currentInterval * exports.PSI));
    }
}
exports.phiAdaptiveInterval = phiAdaptiveInterval;
exports.default = {
    PHI: exports.PHI, PSI: exports.PSI, PHI_SQ: exports.PHI_SQ, PHI_CUBED: exports.PHI_CUBED,
    PHI_CB: exports.PHI_CB, PHI_TEMPERATURE: exports.PHI_TEMPERATURE,
    FIB: exports.FIB, fib,
    ...
    sigmoid, cosineSimilarity, cslGate, cslBlend,
    phiPriorityScore, phiTokenBudgets, phiAdaptiveInterval,
    PRESSURE_LEVELS: exports.PRESSURE_LEVELS, ALERT_THRESHOLDS: exports.ALERT_THRESHOLDS,
    DEDUP_THRESHOLD: exports.DEDUP_THRESHOLD,
