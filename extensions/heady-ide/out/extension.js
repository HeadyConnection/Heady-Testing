...
const HeadyBuddyViewProvider_1 = require("./HeadyBuddyViewProvider");
// ── φ-Math Constants ──────────────────────────────────────────────────────────
...
async function callHeadyAPI(endpoint, body) {
...
function showResultPanel(title, content) {
    const panel = vscode.window.createWebviewPanel('headyResult', title, vscode.ViewColumn.Beside, { enableScripts: false });
...
async function explainCode() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Heady: Explaining code...' }, async () => {
...
async function generateTests() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Heady: Generating tests...' }, async () => {
...
async function fixCode() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Heady: Fixing code...' }, async () => {
...
async function optimizeCode() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Heady: Optimizing code...' }, async () => {
...
async function reviewCode() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Heady: Reviewing code...' }, async () => {
...
function activate(context) {
    ...
    const buddyProvider = new HeadyBuddyViewProvider_1.HeadyBuddyViewProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('heady.buddyChat', buddyProvider));
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('heady.openChat', () => {
        vscode.commands.executeCommand('workbench.view.extension.heady-sidebar');
    }), vscode.commands.registerCommand('heady.explainCode', explainCode), vscode.commands.registerCommand('heady.generateTests', generateTests), vscode.commands.registerCommand('heady.fixCode', fixCode), vscode.commands.registerCommand('heady.optimizeCode', optimizeCode), vscode.commands.registerCommand('heady.reviewCode', reviewCode));
    // Status bar
